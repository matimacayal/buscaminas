...
class RobotPlayer:
    ...
    def press_random_cell(self, mine_map, player_map):
    ...
    def ai_count_press_and_flag(self, player_map, mine_map):
        ...
        for row, col in non_empty_cells:
            ...
            flags, covers = self.count_neighbour_flags_and_covers(row, col, player_map)
            ...
            if cell_value == 0 or covers == 0:
                ...
                self.press_neighbour_cells(row, col, player_map, mine_map)
                pass
            elif flags > cell_value:
                ...
                if covers == cell_value - flags:
                    ...
                    self.fill_neighbours_with_flags(row, col, player_map)
                    pass
    ...
    def ai2_2nd_level_counting(self, player_map, mine_map):
        before_covers_n = np.count_nonzero(player_map == COVERED_CELL_CHAR)
        ...
        for row, col in non_empty_cells:
            # print("checking ", row, col)
            n_flags, n_covered = self.count_neighbour_flags_and_covers(row, col, player_map)
            cell_value = int(player_map[row, col])
            # print("cell_value", cell_value)
    ...
    def get_next_move(self, last_move, last_move_result, player_map):
        next_move = ""
